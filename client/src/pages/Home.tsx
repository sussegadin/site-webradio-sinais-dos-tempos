...
export default function Home() {
  ...
  return <main>
    {visible('hero') && <section className="hero-section"><div className="hero-image"><img src="/images/hero.jpeg" alt="Capa principal" /><div className="hero-glow" /></div><div className="container hero-content"><div className="hero-issue">EST. 2014 <span>/</span> TRANSMISSÃO 24H</div><div className="eyebrow">{text('hero_eyebrow', 'UMA VOZ DE ESPERANÇA')}</div><h1>{hero?.title || `${text('hero_title_line', 'Testemunhando a Vinda de Cristo')} ${text('hero_title_accent', '')}`.trim()}</h1><p>{hero?.body || text('hero_description', 'Conectando você com a Profecia e a Esperança Eterna')}</p><div className="hero-actions"><button onClick={toggleAudio} className="primary-button"><Headphones size={18} /> {playing ? 'Pausar rádio' : (hero?.buttonText || 'Ouça ao vivo')}</button><Link href={hero?.buttonUrl || '/blog'} className="ghost-button">Explorar o blog <ArrowRight size={17} /></Link></div></div></section>}
