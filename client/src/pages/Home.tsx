...
export default function Home() {
  ...
  return <main>
    {visible('hero') && <section className="hero-section"><div className="hero-image"><img src={hero?.imageUrl || '/images/hero.jpeg'} alt="Capa principal" /><div className="hero-glow" /></div><div className="container hero-content"><div className="hero-issue">EST. 2014 <span>/</span> TRANSMISSÃO 24H</div><div className="eyebrow">{text('hero_eyebrow', 'UMA VOZ DE ESPERANÇA')}</div><h1>{hero?.title || `${text('hero_title_line', 'Mensagem que')} ${text('hero_title_accent', 'transforma vidas.')}`}</h1><p>{hero?.body || text('hero_description', 'Ouça, leia e compartilhe conteúdo que aproxima o coração de Deus. A Web Rádio Sinais dos Tempos está com você.')}</p><div className="hero-actions"><button onClick={toggleAudio} className="primary-button"><Headphones size={18} /> {playing ? 'Pausar rádio' : (hero?.buttonText || 'Ouça ao vivo')}</button><Link href={hero?.buttonUrl || '/blog'} className="ghost-button">Explorar o blog <ArrowRight size={17} /></Link></div></div></section>}
    ...
    {visible('blog') && <section className="section container"><div className="section-heading"><div><span className="section-kicker">{text('blog_kicker', 'PALAVRAS PARA O CAMINHO')}</span><h2>Da redação</h2></div><Link href="/blog" className="text-link">Ver todas <ArrowRight size={16} /></Link></div><div className="post-grid">{posts.map((post: any, index: number) => <Link href={`/post/${post.slug}`} className={`post-card card-${index}`} key={post.slug}><div className="post-art">{post.imageUrl && <img src={post.imageUrl} alt="" />}<div className="post-art-glow" /><span>{String(index + 1).padStart(2, '0')} · {post.category}</span></div><div className="post-card-body"><h3>{post.title}</h3><p>{post.summary}</p><span className="read-more">Ler matéria <ArrowRight size={14} /></span></div></Link>)}</div></section>}
