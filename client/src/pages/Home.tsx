...
export default function Home() {
  ...
  return <main>
    ...
    {visible('blog') && <section className="section container"><div className="section-heading"><div><span className="section-kicker">{text('blog_kicker', 'PALAVRAS PARA O CAMINHO')}</span><h2>Da redação</h2></div><nav className="home-category-nav" aria-label="Categorias do blog"><Link href="/blog?categoria=Reflexão" className="home-category-button">Reflexão</Link><Link href="/blog?categoria=Programação" className="home-category-button">Programação</Link><Link href="/blog?categoria=Louvores" className="home-category-button">Louvores</Link><Link href="/blog?categoria=Testemunhos" className="home-category-button">Testemunhos</Link></nav></div><div className="post-grid post-grid-featured">{posts.filter((post: any) => String(post.category || '').toLowerCase() === 'reflexão').slice(0, 1).map((post: any) => <Link href={`/post/${post.slug}`} className="post-card post-card-featured" key={post.slug}><div className="post-art">{post.imageUrl && <img src={post.imageUrl} alt="" />}<div className="post-art-glow" /><span>01 · {post.category}</span></div><div className="post-card-body"><h3>{post.title}</h3><p>{post.summary}</p><span className="read-more">Ler reflexão <ArrowRight size={14} /></span></div></Link>)}</div></section>}
