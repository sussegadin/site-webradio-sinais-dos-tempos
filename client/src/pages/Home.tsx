...
export default function Home() {
  ...
  let blocks: Block[] = [];
  try { if (blocksData?.home_blocks) blocks = JSON.parse(blocksData.home_blocks); } catch { blocks = []; }
  ...
  return <main>
    {visible('hero') && <section className="hero-section"><div className="hero-image"><img src={hero?.imageUrl || copy?.hero_image_url || '/images/hero.jpeg'} alt="Capa principal" /><div className="hero-glow" /></div><div className="container hero-content"><div className="eyebrow"><Sparkles size={15} /> {text('hero_eyebrow', 'UMA VOZ DE ESPERANÇA')}</div><h1>{copy?.hero_title_line || hero?.title || `${text('hero_title_line', 'Mensagem que')} ${text('hero_title_accent', 'transforma vidas.')}`}</h1><p>{copy?.hero_description || hero?.body || text('hero_description', 'Ouça, leia e compartilhe conteúdo que aproxima o coração de Deus. A Web Rádio Sinais dos Tempos está com você.')}</p><div className="hero-actions"><button onClick={toggleAudio} className="primary-button"><Headphones size={18} /> {playing ? 'Pausar rádio' : (copy?.hero_button_text || hero?.buttonText || 'Ouça ao vivo')}</button><Link href={copy?.hero_button_url || hero?.buttonUrl || '/blog'} className="ghost-button">Explorar o blog <ArrowRight size={17} /></Link></div></div></section>}
    {visible('radio') && <section id="ouvir" className="radio-card-wrap"><div className="container"><div className="radio-card"><div className="radio-card-icon"><Headphones size={25} /></div><div><span className="section-kicker">TRANSMISSÃO ONLINE</span><h2>{radio?.title || 'Uma programação para acompanhar você'}</h2><p>{radio?.body || 'Pregações, louvores e mensagens proféticas durante todo o dia.'}</p></div><div className="radio-wave" aria-hidden="true"><i /><i /><i /><i /><i /><i /><i /><i /></div><div className="radio-note">Aperte <strong>Ouvir ao vivo</strong> no topo da página</div></div></div></section>}
    {visible('blog') && <section className="section container"><div className="section-heading"><div><span className="section-kicker">{text('blog_kicker', 'PALAVRAS PARA O CAMINHO')}</span></div><Link href="/blog" className="text-link">Ver todas <ArrowRight size={16} /></Link></div><div className="post-grid">{posts.map((post: any, index: number) => <Link href={`/post/${post.slug}`} className={`post-card card-${index}`} key={post.slug}><div className="post-art"><div className="post-art-glow" /><span>{post.category}</span></div><div className="post-card-body"><h3>{post.title}</h3><p>{post.summary}</p><span className="read-more">Ler matéria <ArrowRight size={14} /></span></div></Link>)}</div></section>}
    ...
    {visible('mission') && <section className="mission-section"><div className="container mission-grid"><div><span className="section-kicker">{text('mission_kicker', 'NOSSA MISSÃO')}</span><h2>{mission?.title || text('mission_title', 'Conteúdo que aponta para o alto.')}</h2></div><p>{mission?.body || text('mission_description', 'Em cada palavra, canção e transmissão, buscamos levar paz, conhecimento bíblico e a esperança do retorno de Cristo.')}</p><Link href="/blog" className="primary-button">Conheça o projeto <ArrowRight size={17} /></Link></div></section>}
  </main>;
}
