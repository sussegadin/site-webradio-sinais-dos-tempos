...
export default function Editor() {
  ...
  const save = useMutation({ mutationFn: ({ status, content }: { status: 'draft' | 'published'; content: string }) => apiFetch(editing ? `/api/posts/${editing}` : '/api/posts', { method: editing ? 'PATCH' : 'POST', body: JSON.stringify({ ...form, content, status, imageUrl: form.imageUrl || null }) }), onSuccess: () => { setForm(empty); setEditing(null); setError(''); setNotice('Matéria salva com sucesso.'); if (bodyRef.current) bodyRef.current.innerHTML = ''; client.invalidateQueries({ queryKey: ['admin-posts'] }); client.invalidateQueries({ queryKey: ['posts'] }); } });
  ...
  return <main className="container admin-main"><div className="admin-header"><div><span className="section-kicker">PAINEL ADMINISTRATIVO</span><h1>Editor de matérias</h1><p>{editing ? 'Edite, cole o texto e publique sem perder a estrutura.' : 'Cole o texto da matéria, adicione imagens e publique.'}</p></div><Link href="/admin" className="text-link">Voltar ao painel</Link></div>
    <div className="editor-layout"><section className="editor-card rich-editor-card"><input className="editor-title" value={form.title} onChange={e=>set('title',e.target.value)} placeholder="Título da matéria"/><input className="editor-summary" value={form.summary} onChange={e=>set('summary',e.target.value)} placeholder="Resumo curto para o blog"/><input className="editor-summary" value={form.category} onChange={e=>set('category',e.target.value)} placeholder="Categoria"/>
      ...
      {uploadingImage && <p className="rich-editor-status">Enviando imagem colada...</p>}{error&&<p className="form-error">{error}</p>}{notice&&<p className="form-success">{notice}</p>}<div className="editor-actions"><button className="ghost-button" onClick={reset}><X size={16}/> {editing?'Cancelar':'Limpar'}</button><button className="ghost-button" onClick={()=>publish('draft')} disabled={save.isPending||!form.title}><Save size={16}/> Salvar rascunho</button><button className="primary-button" onClick={()=>publish('published')} disabled={save.isPending||!form.title}><Send size={16}/> {editing?'Salvar e publicar':'Publicar matéria'}</button></div>
