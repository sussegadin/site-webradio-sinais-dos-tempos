...
export default function Blog(){
  ...
  return <main className="container page-main"><div className="page-intro"><span className="section-kicker">REFLEXÕES • NOTÍCIAS • LOUVORES</span><h1>Blog Sinais dos Tempos</h1><p>Conteúdo para fortalecer a fé e iluminar o caminho.</p></div><div className="blog-toolbar"><div className="search-box"><Search size={18}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar no blog..." aria-label="Buscar no blog"/></div><div className="category-pills" role="group" aria-label="Filtrar conteúdo">{categories.map(item=><button type="button" key={item} className={category===item?'active':''} onClick={()=>setCategory(item)} aria-pressed={category===item}>{item}</button>)}</div></div>{category==='Testemunhos'?<section className="blog-testimonials-list" aria-label="Testemunhos aprovados">{shownTestimonials.map((item:any)=><article className="blog-testimonial" key={item.id}><div className="blog-testimonial-mark">“</div><div><p>{item.content}</p><strong>{item.name}</strong>{item.location&&<small><Calendar size={13}/> {item.location}</small>}</div></article>)}{!shownTestimonials.length&&<div className="empty-state">Ainda não há testemunhos publicados.</div>}</section>:<div className="blog-list">{shown.map((post:any)=><Link href={`/post/${post.slug}`} className="blog-row" key={post.slug}><div className="blog-thumb" style={post.imageUrl?{backgroundImage:`url(${post.imageUrl})`,backgroundSize:'cover',backgroundPosition:'center'}:undefined}><span>{post.category}</span></div><div><span className="post-category">{post.category}</span><h2>{post.title}</h2><p>{post.summary}</p><span className="text-link">Ler matéria <ArrowRight size={15}/></span></div></Link>)}{!shown.length&&<div className="empty-state">Nenhuma matéria encontrada nessa categoria.</div>}</div>}</main>
}
