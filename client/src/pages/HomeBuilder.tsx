...
export default function HomeBuilder() {
  ...
  if (!isAuthenticated) return <main className="container page-main"><div className="locked-card"><LayoutTemplate size={36} /><h1>Editor visual Pro</h1><p>Entre para criar e publicar seu site.</p><Link href="/admin/login" className="primary-button">Entrar</Link></div></main>;
  ...
  return <main className="builder-page">
    <header className="builder-topbar"><div className="builder-brand"><Sparkles size={18} /><strong>Sinais dos Tempos</strong><span>Builder Pro</span></div><div className="builder-top-actions"><Link href="/" className="builder-preview"><Eye size={16} /> Pré-visualizar</Link><button type="button" className="builder-publish" onClick={() => save.mutate(blocks)} disabled={save.isPending}><Save size={16} /> {save.isPending ? 'Publicando...' : 'Publicar'}</button><Link href="/admin" className="builder-close" aria-label="Fechar editor"><X size={18} /></Link></div></header>
