...
type MediaAsset = { key: string; url: string; size: number; uploaded: string };
...
export default function HomeBuilder() {
  ...
  const media = useQuery<{ items: MediaAsset[] }>({ queryKey: ['admin-media'], queryFn: () => apiFetch('/api/media/list'), enabled: isAuthenticated });
  useEffect(() => {
  ...
  const upload = useMutation({ mutationFn: (value: any) => apiFetch<{ url: string }>('/api/media/upload', { method: 'POST', body: JSON.stringify(value) }), onSuccess: () => media.refetch() });
  ...
  const useAsset = (asset: MediaAsset) => {
    if (!draft) { setError('Selecione primeiro uma seção no Canvas.'); setActivePanel('canvas'); return; }
    setDraftField('imageUrl', asset.url);
    setActivePanel('canvas');
    setNotice(`Imagem selecionada para “${draft.label}”. Clique em Aplicar e depois em Publicar.`);
  };
  ...
  return <main className="builder-page">
    ...
    <div className="builder-layout">
      <aside className="builder-sidebar builder-left"><div className="builder-tabs"><button className={activePanel === 'canvas' ? 'active' : ''} onClick={() => setActivePanel('canvas')}><LayoutTemplate size={16} /> Canvas</button><button className={activePanel === 'assets' ? 'active' : ''} onClick={() => setActivePanel('assets')}><FolderOpen size={16} /> Assets</button><button className={activePanel === 'settings' ? 'active' : ''} onClick={() => setActivePanel('settings')}><Settings2 size={16} /> Configurações</button></div>
        ...
        {activePanel === 'assets' && <div className="builder-panel-content"><div className="builder-sidebar-heading"><div><strong>Biblioteca</strong><small>Selecione uma imagem para o elemento ativo</small></div></div><input ref={mediaRef} type="file" accept="image/*,video/*" hidden onChange={chooseMedia} /><button type="button" className="builder-upload-card" onClick={() => mediaRef.current?.click()}><Upload size={22} /><strong>Enviar arquivos</strong><small>Imagens até 16 MB · vídeos até 64 MB</small></button>{media.isLoading && <p className="builder-feature-note">Carregando biblioteca...</p>}{media.data?.items?.length ? <div className="builder-assets-grid">{media.data.items.filter(asset => asset.key.startsWith('images/')).map(asset => <button type="button" className="builder-asset-card" key={asset.key} onClick={() => useAsset(asset)} title="Usar esta imagem no elemento selecionado"><img src={asset.url} alt="" /><span>Usar imagem</span></button>)}</div> : !media.isLoading && <div className="builder-empty builder-assets-empty"><ImagePlus size={25} /><p>Nenhuma imagem enviada ainda.</p><small>Envie uma imagem acima para ela aparecer aqui.</small></div>}<div className="builder-feature-note"><Video size={16} /><span>Os arquivos enviados ficam disponíveis nesta biblioteca e podem ser usados novamente.</span></div></div>}
