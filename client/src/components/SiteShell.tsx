...
export default function SiteShell({ children }: { children: ReactNode }) {
  ...
  return <div className="site-shell">
    ...
    <div className="live-strip"><div className="container live-player"><div className="live-player-brand"><div className={playing ? 'live-player-icon playing' : 'live-player-icon'}><Radio size={20} /></div><div className="live-copy"><span className="live-status"><span className={playing ? 'live-dot active' : 'live-dot'}></span>{playing ? 'NO AR AGORA' : 'RÁDIO ONLINE'}</span><small>Sinais dos Tempos Web Rádio <b>•</b> A Rádio dos Remanescentes</small></div></div><div className={playing ? 'live-equalizer active' : 'live-equalizer'} aria-hidden="true"><i /><i /><i /><i /><i /><i /><i /></div><div className="live-player-action"><Volume2 size={15} className="live-volume-icon" /><button className={playing ? 'live-button playing' : 'live-button'} onClick={toggleAudio} aria-label={playing ? 'Pausar rádio ao vivo' : 'Ouvir rádio ao vivo'}>{playing ? <Pause size={15} fill="currentColor" /> : <Play size={15} fill="currentColor" />}{playing ? 'Pausar rádio' : 'Ouvir ao vivo'}</button></div></div></div>
